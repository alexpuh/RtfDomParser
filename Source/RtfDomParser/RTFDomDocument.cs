            ...
            foreach (RTFDomElement element in parentElement.Elements)
            {
                ...
                if (element is RTFDomText)
                {
                    if (result.Count > 0 && result[result.Count - 1] is RTFDomText)
                    {
                        ...
                        {
                            // only combine text runs that originate from the same RTF brace
                            // group; identical formatting alone must not merge across a
                            // "}{" group boundary, since that boundary is a hard scope
                            // delimiter in RTF and callers may rely on it as a segment split.
                            if (lastText.Format.EqualsSettings(txt.Format) && lastText.GroupId == txt.GroupId)
                            {
                                lastText.Text = lastText.Text + txt.Text;
                            }
                            else
        ...
        private void Load(RTFReader reader , DocumentFormatInfo parentFormat)
        {
            ...
            while (reader.ReadToken() != null)
            {
                ...
                if (bolStartContent)
                {
                    ...
                    {
                        if (ApplyText(myText, reader, format, groupId))
                        {
                            break;
                        }
                    }
            ...
            if (myText.HasContent)
            {
                ApplyText(myText, reader, format, groupId);
            }
        }
